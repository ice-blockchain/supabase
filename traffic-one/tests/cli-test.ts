...
async function fetchScopedTokenById(id: string): Promise<ScopedTokenDbRow | null> {
...
Deno.test("POST /cli/login repeated calls issue fresh tokens with different token values", async () => {
  const session = await getTestSession();
