...
async function signUpDisposableUser(): Promise<
  { email: string; password: string }
> {
  const email = `update-email-${Date.now()}-${Math.floor(Math.random() * 1e6)}@example.com`;
  ...
  // Force-confirm the user in case ENABLE_EMAIL_AUTOCONFIRM is false so we can
  // sign in immediately. Safe no-op if the user is already confirmed.
...
Deno.test("PUT /update-email returns 401 without auth", async () => {
  const res = await fetch(UPDATE_EMAIL_URL, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ newEmail: "does-not-matter@example.com", hcaptchaToken: null }),
  });
...
Deno.test("PUT /update-email with invalid email returns 400", async () => {
  ...
  const res = await fetch(UPDATE_EMAIL_URL, {
    ...
    body: JSON.stringify({ newEmail: "not-a-valid-email", hcaptchaToken: null }),
  });
...
Deno.test(
  ...
  async () => {
    const { email: originalEmail, password } = await signUpDisposableUser();
    const session = await signIn(originalEmail, password);
    ...
    const newEmail =
      `update-email-new-${Date.now()}-${Math.floor(Math.random() * 1e6)}@example.com`;
    ...
    const newSession = await signIn(newEmail, password);
    ...
    const match = auditBody.result.find(
      (row: { action: { name: string } }) =>
        row.action?.name === "profile.email_updated",
    );
    assertExists(
      match,
