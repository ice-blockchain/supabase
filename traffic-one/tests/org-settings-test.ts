...
Deno.test('GET /organizations/{slug}/audit returns 401 without auth', async () => {
  const res = await fetch(
    `${ORG_URL}/${testSlug}/audit?iso_timestamp_start=2024-01-01T00:00:00Z&iso_timestamp_end=2025-01-01T00:00:00Z`,
  )
...
Deno.test('MFA toggle creates an audit log entry', async () => {
  ...
  assertEquals(res.status, 200)
  const body = await res.json()
  const mfaEntry = body.result.find((e: { action: { name: string } }) =>
    e.action.name === 'organizations.mfa_update'
  )
