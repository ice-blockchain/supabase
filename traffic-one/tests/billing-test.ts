...
Deno.test("GET /organizations/{slug}/tax-ids returns array", async () => {
  if (!testSlug) return;
  const session = await getTestSession();
  ...
  const taxIds = await res.json();
  assert(Array.isArray(taxIds));
});

Deno.test("PUT /organizations/{slug}/tax-ids creates a tax ID", async () => {
  if (!testSlug) return;
  const session = await getTestSession();
  const res = await fetch(`${ORG_URL}/${testSlug}/tax-ids`, {
    method: "PUT",
    headers: authHeaders(session.access_token),
    body: JSON.stringify({ type: "eu_vat", value: "DE123456789" }),
  });
  assertEquals(res.status, 200);

  const taxId = await res.json();
  assertEquals(taxId.type, "eu_vat");
  assertEquals(taxId.value, "DE123456789");
  assertExists(taxId.id);
});
