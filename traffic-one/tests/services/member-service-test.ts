...
async function createTestProfile(
  ...
  const result = await tx.queryObject<{ id: number; gotrue_id: string }>`
    ...
    VALUES (${'00000000-0000-0000-0000-000000mem' + suffix}, ${'memuser' + suffix}, ${
    suffix + '@memtest.com'
  })
...
Deno.test('roles table contains 4 seeded roles', async () => {
  await pool.withConnection(async (connection) => {
    const result = await connection.queryObject<
      { id: number; name: string; base_role_id: number }
    >`
...
Deno.test(
  'organization_member_roles unique constraint prevents duplicate role assignment',
  async () => {
    await pool.withConnection(async (connection) => {
    ...
    `
      ...
      assert(
        threw,
        'Duplicate role assignment should throw a constraint error',
      )
...
Deno.test('invitation has 24h expiry by default', async () => {
  await pool.withConnection(async (connection) => {
    ...
    assert(
      diffHours >= 23.9 && diffHours <= 24.1,
      `Expected ~24h expiry, got ${diffHours}h`,
    )
...
Deno.test('deleting organization cascades to member_roles and invitations', async () => {
  await pool.withConnection(async (connection) => {
    ...
    assertEquals(
      roles.rows[0].cnt,
      0,
      'Member roles should be cascade-deleted',
    )
    ...
    `
    assertEquals(
      invitations.rows[0].cnt,
      0,
      'Invitations should be cascade-deleted',
    )
...
Deno.test('organization_member_roles stores project_refs', async () => {
  await pool.withConnection(async (connection) => {
    ...
    await tx.queryObject`
      ...
      VALUES (${orgId}, ${profile.id}, 3, ${['proj-a', 'proj-b']})
    `
