...
async function createTestProfile(
  ...
  const result = await tx.queryObject<{ id: number; gotrue_id: string }>`
    ...
    VALUES (${"00000000-0000-0000-0000-000000set" + suffix}, ${"setuser" + suffix}, ${suffix + "@settings.com"})
...
Deno.test("SSO provider insert and select by organization_id", async () => {
  ...
  try {
    ...
    const sso = await tx.queryObject<{ id: string; enabled: boolean; domains: string[] }>`
...
Deno.test("SSO provider unique constraint prevents duplicate per org", async () => {
  ...
  try {
    ...
    assert(threw, "Duplicate SSO provider for same org should throw a constraint error");
...
Deno.test("SSO provider update changes fields", async () => {
  ...
  try {
    ...
    const updated = await tx.queryObject<{ enabled: boolean; metadata_xml_url: string }>`
    ...
    assertEquals(updated.rows[0].metadata_xml_url, "https://new.example.com/metadata");
...
Deno.test("deleting organization cascades to sso_providers", async () => {
  ...
  try {
    ...
    `;
    assertEquals(remaining.rows.length, 0, "SSO provider should be cascade-deleted with org");
