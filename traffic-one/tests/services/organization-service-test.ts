...
async function createTestProfile(
  ...
  const result = await tx.queryObject<{ id: number }>`
    ...
    VALUES (${"00000000-0000-0000-0000-0000000org" + suffix}, ${"orguser" + suffix}, ${suffix + "@orgtest.com"})
    RETURNING id
  `;
...
Deno.test("insert organization and select by slug", async () => {
  ...
  try {
    ...
    const orgResult = await tx.queryObject<{ id: number; slug: string; name: string }>`
...
Deno.test("update organization name and billing_email", async () => {
  ...
  try {
    ...
    const updated = await tx.queryObject<{ name: string; billing_email: string | null }>`
