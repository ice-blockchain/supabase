...
import { getOrgUsage, getOrgDailyUsage } from "../../functions/services/usage.service.ts";
import {
  getDefaultPricing,
  getEffectivePricing,
  calculateCost,
...
import type { PricingOverride, MetricPricing } from "../../functions/types/api.ts";
...
Deno.test("calculateCost: PACKAGE strategy charges ceil(overage/package_size) * package_price", () => {
  const pricing: MetricPricing = {
...
Deno.test("getEffectivePricing: per-metric discount reduces price", () => {
  const overrides: PricingOverride[] = [{
    id: 1, organization_id: 1, metric: "EGRESS",
    discount_percent: 50, custom_free_units: null, custom_per_unit_price: null, notes: null,
  }];
...
Deno.test("getEffectivePricing: global discount applies when no per-metric override", () => {
  const overrides: PricingOverride[] = [{
    id: 1, organization_id: 1, metric: null,
    discount_percent: 20, custom_free_units: null, custom_per_unit_price: null, notes: null,
  }];
...
Deno.test("getEffectivePricing: per-metric override takes priority over global", () => {
  const overrides: PricingOverride[] = [
    { id: 1, organization_id: 1, metric: null, discount_percent: 10, custom_free_units: null, custom_per_unit_price: null, notes: null },
    { id: 2, organization_id: 1, metric: "EGRESS", discount_percent: 50, custom_free_units: null, custom_per_unit_price: null, notes: null },
  ];
  const base = getDefaultPricing("pro", "EGRESS");
  const effective = getEffectivePricing("pro", "EGRESS", overrides);
  assertEquals(effective.per_unit_price, base.per_unit_price * 0.5);
...
Deno.test("getEffectivePricing: custom_free_units overrides default", () => {
  const overrides: PricingOverride[] = [{
    id: 1, organization_id: 1, metric: "EGRESS",
    discount_percent: 0, custom_free_units: 999999, custom_per_unit_price: null, notes: null,
  }];
  const effective = getEffectivePricing("pro", "EGRESS", overrides);
...
Deno.test("getEffectivePricing: custom_per_unit_price overrides default", () => {
  const overrides: PricingOverride[] = [{
    id: 1, organization_id: 1, metric: "MONTHLY_ACTIVE_USERS",
    discount_percent: 0, custom_free_units: null, custom_per_unit_price: 0.002, notes: null,
  }];
  const effective = getEffectivePricing("pro", "MONTHLY_ACTIVE_USERS", overrides);
...
Deno.test("getOrgUsage returns correct structure with usage_billing_enabled true", async () => {
  ...
  try {
    ...
    try {
      await cleanConn.queryObject`DELETE FROM traffic.organizations WHERE id = ${orgId}`;
    } finally {
      cleanConn.release();
    }
  } catch (err) {
    try { await tx.rollback(); } catch { /* already committed or rolled back */ }
    throw err;
  } finally {
    connection.release();
  }
