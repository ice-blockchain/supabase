...
async function createTestOrg(
  ...
  const profile = await tx.queryObject<{ id: number }>`
    ...
    VALUES (${"00000000-0000-0000-0000-000bill" + suffix}, ${"billuser" + suffix}, ${suffix + "@billtest.com"})
    RETURNING id
  `;
...
Deno.test("update subscription tier", async () => {
  ...
  try {
    ...
    const updated = await tx.queryObject<{ plan_id: string; plan_name: string; tier: string }>`
...
Deno.test("customer upsert inserts and updates", async () => {
  ...
  try {
    ...
    const inserted = await tx.queryObject<{ billing_name: string; country: string }>`
    ...
    const updated = await tx.queryObject<{ billing_name: string; city: string }>`
...
Deno.test("tax ID insert and delete", async () => {
  ...
  try {
    ...
    const inserted = await tx.queryObject<{ id: number; type: string; value: string }>`
