...
async function createTestProfile(
  ...
  const result = await tx.queryObject<{ id: number }>`
    ...
    VALUES (${"00000000-0000-0000-0000-000000proj" + suffix}, ${"projuser" + suffix}, ${suffix + "@projtest.com"})
    RETURNING id
  `;
...
Deno.test("insert project and select by ref", async () => {
  ...
  try {
    ...
    const result = await tx.queryObject<{ id: number; ref: string; name: string; status: string }>`
