...
Deno.test('GET /organizations/{slug}/usage returns OrgUsageResponse shape', async () => {
  ...
  assert(
    dbSizeEntry.usage > 0,
    'DATABASE_SIZE usage should be > 0 (real data)',
  )
...
// H1 regression: `?project_ref=…` used to be passed straight into
// `getProjectBackend(ref)` with no membership check. An authenticated member
// of org A could pass `project_ref` pointing at a project owned by org B and
// silently scope usage / Logflare queries against org B's backend. The fix
// calls `getProjectByRef(pool, projectRef, profileId)` and also asserts
// `project.organization_id === org.id` before resolving the backend; a
// mismatch returns 404 (anti-enumeration, not 403). This test pins the 404
// and confirms the response body does NOT carry usage rows for the
// cross-org project.
Deno.test(
  'H1: GET /organizations/{slug}/usage?project_ref=<other-org-project> returns 404',
  async () => {
    if (!testSlug) return
    const session = await getTestSession()
    // "default" is the local-stack default project and belongs to the seeded
    // default org, not to our freshly created `testSlug` org, so passing it
    // as a cross-org project_ref must be rejected.
    const res = await fetch(
      `${ORG_URL}/${testSlug}/usage?project_ref=aaaaaaaaaaaaaaaaaaaa`,
      {
        headers: authHeaders(session.access_token),
      },
    )
    assert(
      ...
      `cross-org project_ref must be denied (got ${res.status})`,
    )
    await res.body?.cancel()
  },
)
...
Deno.test(
  'H1: GET /organizations/{slug}/usage/daily?project_ref=<other-org-project> returns 404',
  async () => {
    if (!testSlug) return
    const session = await getTestSession()
    const now = new Date()
    ...
    const res = await fetch(
      `${ORG_URL}/${testSlug}/usage/daily?project_ref=aaaaaaaaaaaaaaaaaaaa&start=${
        encodeURIComponent(
          start,
        )
    ...
    assert(
      res.status === 404 || res.status === 403,
      `cross-org project_ref must be denied on daily endpoint (got ${res.status})`,
    )
    await res.body?.cancel()
  },
)
...
Deno.test('GET /organizations/{slug}/usage/daily returns OrgDailyUsageResponse shape', async () => {
  ...
  const res = await fetch(
    `${ORG_URL}/${testSlug}/usage/daily?start=${encodeURIComponent(start)}&end=${
      encodeURIComponent(
        end,
      )
    }`,
...
Deno.test('GET /organizations/{slug}/usage/daily returns 404 for nonexistent org', async () => {
  const session = await getTestSession()
  const res = await fetch(
    `${ORG_URL}/nonexistent-org-slug-usage-99999/usage/daily`,
    {
      headers: authHeaders(session.access_token),
    },
  )
