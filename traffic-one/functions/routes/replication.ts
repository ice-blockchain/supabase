...
export async function handleReplication(
  ...
  const destPipelineMatch = subPath.match(
    /^\/destinations-pipelines\/([^/]+)\/([^/]+)$/,
  );
  ...
  const pipelineReplicationStatusMatch = subPath.match(
    /^\/pipelines\/([^/]+)\/replication-status$/,
  );
  ...
  const pipelineActionMatch = subPath.match(
    /^\/pipelines\/([^/]+)\/(start|stop|rollback-tables)$/,
  );
