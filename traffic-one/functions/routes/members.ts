...
export async function handleMembers(
  ...
  if (memberPath === "/mfa/enforcement" && method === "PATCH") {
    ...
    const mfa = await updateMfaEnforcement(pool, orgId, body.enforced, profileId, gotrueId, auditCtx);
    return Response.json(mfa, { headers: corsHeaders });
  }
  ...
  if (memberPath === "/invitations" && method === "POST") {
    ...
    if (!body.email || !body.role_id) {
      return Response.json({ message: "email and role_id are required" }, { status: 400, headers: corsHeaders });
    }
    const result = await createInvitation(pool, orgId, body, profileId, gotrueId, auditCtx);
    if (result.error) {
      return Response.json({ message: result.error }, { status: result.status ?? 400, headers: corsHeaders });
    }
  ...
  if (invDeleteMatch && method === "DELETE") {
    ...
    if (!deleted) {
      return Response.json({ message: "Invitation not found" }, { status: 404, headers: corsHeaders });
    }
  ...
  if (memberRoleMatch) {
    ...
    if (method === "PUT") {
      ...
      const result = await updateMemberRole(
        pool, orgId, targetGotrueId, roleId, body.role_scoped_projects ?? [], profileId, gotrueId, auditCtx,
      );
      if (!result.success) {
        return Response.json({ message: result.error }, { status: result.status ?? 400, headers: corsHeaders });
      }
    ...
    if (method === "DELETE") {
      ...
      const result = await unassignMemberRole(pool, orgId, targetGotrueId, roleId, profileId, gotrueId, auditCtx);
      if (!result.success) {
        return Response.json({ message: result.error }, { status: result.status ?? 400, headers: corsHeaders });
      }
  ...
  if (memberPatchMatch && method === "PATCH") {
    ...
    if (!body.role_id) {
      return Response.json({ message: "role_id is required" }, { status: 400, headers: corsHeaders });
    }
    const result = await assignMemberRole(
      pool, orgId, targetGotrueId, body.role_id, body.role_scoped_projects, profileId, gotrueId, auditCtx,
    );
    if (!result.success) {
      return Response.json({ message: result.error }, { status: result.status ?? 400, headers: corsHeaders });
    }
