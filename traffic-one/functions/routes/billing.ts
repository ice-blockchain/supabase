...
export async function handleBilling(
  ...
  if (subPath === "/tax-ids" && method === "GET") {
    const taxIds = await listTaxIds(pool, orgId);
    return Response.json(taxIds, { headers: corsHeaders });
  }

  if (subPath === "/tax-ids" && method === "PUT") {
    const body = await req.json();
    const taxId = await upsertTaxId(pool, orgId, body.type, body.value);
...
export async function handleStripe(
  ...
  if (subPath === "/setup-intent" && method === "POST") {
    ...
    return Response.json({ id: "", client_secret: "" }, { headers: corsHeaders });
  }
...
export async function handleProjectBilling(
  ...
  if (subPath === "/billing/addons" && method === "POST") {
    ...
    const addons = await applyProjectAddon(
      pool, ref, body.addon_type ?? body.type, body.addon_variant ?? body.variant,
    );
    return Response.json(addons, { headers: corsHeaders });
  }
...
export async function handleConfirmSubscription(
  _req: Request,
  _method: string,
): Promise<Response> {
