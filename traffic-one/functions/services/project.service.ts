...
// Project transfers are destructive (they change ownership/billing/auditing of
// an entire project tree). Upstream only shows the "move" button to admins and
// owners, but here it's a platform-layer call so we enforce the same role gate
// server-side: the caller must hold role_id >= 4 (Administrator or Owner) in
// BOTH the source and target orgs. Returning a distinct `forbidden` result
// lets the route return 403 instead of masking the failure as 404.
const MIN_ROLE_FOR_TRANSFER = 4

export type TransferPreviewResult =
  | { valid: true }
  | { valid: false; forbidden?: boolean; message: string }

async function highestRoleInOrg(
  tx: {
...
export async function transferProjectPreview(
  ...
  try {
    ...
    const targetRole = await highestRoleInOrg(connection, targetOrg.rows[0].id, profileId)
    if (targetRole < MIN_ROLE_FOR_TRANSFER) {
      return {
        valid: false,
        forbidden: true,
        message: 'You must be an administrator or owner of the target organization',
      }
    }
...
export type TransferProjectResult =
  | { ok: true; project: RemoveProjectResponse }
...
export async function transferProject(
  ...
  try {
    ...
    const sourceRole = await highestRoleInOrg(tx, projectResult.rows[0].organization_id, profileId)
    if (sourceRole < MIN_ROLE_FOR_TRANSFER) {
      await tx.rollback()
    ...
    const targetRole = await highestRoleInOrg(tx, targetOrg.rows[0].id, profileId)
    if (targetRole < MIN_ROLE_FOR_TRANSFER) {
      await tx.rollback()
      return { ok: false, forbidden: true }
    }
    ...
    return {
      ok: true,
      project: { id: project.id, ref: project.ref, name: project.name, status: project.status },
    }
  } finally {
    connection.release()
  }
